...
export function openRepository(path: string): Repository {
  return createRepository(new Database(path));
}
function createRepository(db: Database.Database): Repository {
  ...
  const setWork = (item: WorkItem, changes: Partial<WorkItem>) => {
    return db.prepare("UPDATE work_items SET status=@status,validation_evidence=@validation_evidence,updated_at=@updated_at,closed_at=@closed_at,commit_hash=@commit_hash WHERE id=@id").run({ ...item, ...changes, updated_at: now() });
  };
  ...
  const materializePlan = db.transaction((plan: ApprovedPlan) => {
    ...
    if (db.prepare("SELECT 1 FROM work_items WHERE status <> 'closed'").get()) {
    ...
    insertTasks(item.id, 1, plan.tasks, timestamp);
    return item;
  });
  const materializeRework = db.transaction((workId: string) => {
    const item = getWork(workId);
    const review = item ? db.prepare("SELECT * FROM work_reviews WHERE work_item_id=? ORDER BY id DESC LIMIT 1").get(item.id) as Review | undefined : undefined;
    ...
    const first = (db.prepare("SELECT COALESCE(MAX(position), 0) AS position FROM tasks WHERE work_item_id = ?").get(item.id) as { position: number }).position + 1;
  ...
  const createCheckpoint = db.transaction((input: Omit<Checkpoint, "id" | "created_at">) => { const item = getWork(input.work_item_id); const task = input.task_id ? getTask(input.task_id) : null; if (!item || item.status !== "active" || (input.task_id && (!task || task.work_item_id !== item.id || task.status !== "active"))) throw new Error("Checkpoint requires an active Work and, when supplied, its active Task."); const result = db.prepare("INSERT INTO checkpoints (work_item_id,task_id,summary,next_step,created_at) VALUES (?,?,?,?,?)").run(item.id, input.task_id, input.summary, input.next_step, now()); return db.prepare("SELECT * FROM checkpoints WHERE id=?").get(result.lastInsertRowid) as Checkpoint; });
  const closeWork = db.transaction((workId: string, commitHash: string | null) => {
    const item = getWork(workId);
    const latest = item ? db.prepare("SELECT outcome FROM work_reviews WHERE work_item_id=? ORDER BY id DESC LIMIT 1").get(item.id) as { outcome: string } | undefined : undefined;
