...
import { createIntake, createPlanningEntrypoint, createProposal, getIntake, getProposal, intakeStatus, readIntentInput, reviewProposal, verifyIntake } from "./intake.js";
...
intakeCommand.command("status").argument("<intakeId>").description("Show Intake state and proposal history for resumption.").action((id: string) => { const status = getInitializedWorkspaceStatus(process.cwd()); if (!status.databasePath) return; try { const result = intakeStatus(status.databasePath, id); console.log(`${result.intake.id}: ${result.intake.status}`); for (const proposal of result.proposals) console.log(`  ${proposal.id}: ${proposal.status}`); } catch (error) { program.error(error instanceof Error ? error.message : String(error), { code: "NERV_INTAKE_NOT_FOUND", exitCode: 1 }); } });
