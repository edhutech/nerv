...
import { FINDING_SEVERITIES, parseReviewFindings, type ReviewFinding } from "./findings.js";
...
const REVIEW_FINDINGS_EXAMPLE = JSON.stringify([{ severity: "high", issue: "Describe the finding", pass_impact: "This finding blocks PASS because the approved outcome is not met.", evidence: "Relevant validation or review evidence.", affected_work_criterion: "The affected Work acceptance criterion." }]);
...
function reviewWork(reference: string, options: { outcome: string; summary: string; validationEvidence: string; verificationEvidence?: string; findings?: string } & RemediationOptions) {
  ...
  try {
    ...
    const findings = parseReviewFindings(options.findings);
