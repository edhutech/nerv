...
export function activePath(workspaceRoot: string, workId: string) { return join(workspaceRoot, "agent", "active", `${workId}.md`); }
export function syncActiveContext(workspaceRoot: string, work: WorkItem, tasks: Task[], review: Review | null, checkpoint: Checkpoint | null): string {
  const taskList = (status: string) => tasks.filter((task) => task.status === status).map((task) => `- ${task.id}: ${task.title}`).join("\n") || "- None";
  const active = tasks.find((task) => task.status === "active");
  const next = nextOperation(work, tasks, review);
  const content = `# ${work.id}: ${work.title}\n\n## State\n\n${work.status}\n\n## Goal\n\n${work.goal}\n\n## Acceptance Criteria\n\n${work.acceptance_criteria}\n\n## Validation\n\n${work.validation}\n\n## Completed Tasks\n\n${taskList("done")}\n\n## Pending Tasks\n\n${taskList("pending")}\n\n## Active Task\n\n${active ? `${active.id}: ${active.title}\n\n${active.scope}` : "None"}\n\n## Latest Review\n\n${review ? `${review.outcome}: ${review.summary}\n\n${review.findings ?? "No findings."}` : "None"}\n\n## Latest Checkpoint\n\n${checkpoint ? `${checkpoint.summary}\n\nNext: ${checkpoint.next_step ?? "Not recorded."}` : "None"}\n\n## Next\n\n${next}\n`;
  const path = activePath(workspaceRoot, work.id); writeFileSync(path, content, "utf8"); return path;
}
export function removeActiveContext(workspaceRoot: string, workId: string) { const path = activePath(workspaceRoot, workId); if (existsSync(path)) rmSync(path); }
