...
function remediationProposal(value: string | null) {
  if (!value) return "None";
  try {
    const tasks = JSON.parse(value) as Array<{ title: string; objective: string; implementation_approach: string; expected_touchpoints: string; acceptance_criteria: string; validation: string }>;
    return tasks.map((task, index) => `### Remediation Task ${index + 1}: ${task.title}\n\nObjective: ${task.objective}\n\nImplementation approach: ${task.implementation_approach}\n\nExpected touchpoints: ${task.expected_touchpoints}\n\nAcceptance criteria: ${task.acceptance_criteria}\n\nTargeted validation: ${task.validation}`).join("\n\n") || "None";
  } catch {
    return "Invalid persisted remediation proposal.";
  }
}
export function syncActiveContext(workspaceRoot: string, work: WorkItem, tasks: Task[], review: Review | null, checkpoint: Checkpoint | null): string {
  const taskList = (status: string) => {
    return tasks.filter((task) => task.status === status).map((task) => `- Task ${task.position}: ${task.title}`).join("\n") || "- None";
  };
