...
The UUID remains the durable Work identity; the `W-` alias is a deterministic uppercase, collision-resistant human-readable ref derived from at least 64 UUID bits. SQLite uniqueness is local defense in depth only. `nerv close <work-ref> --message <subject>` preserves both UUID and human-readable ref attribution.
