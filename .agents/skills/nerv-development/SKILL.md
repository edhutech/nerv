...
SQLite is the durable operational source of truth. Do not edit `.nerv/` state or `dist/` directly.
...
Before the `nerv-dev plan` protocol operation, inspect only relevant portions of `.nerv-context/product.md` and `.nerv-context/repo.md`; do not load either file ritualistically. Relevant external context sources such as repository tooling, documentation, skills, MCPs, plugins, or specialized tools may assist when available; they are optional and do not replace Nerv governance. Plan may propose context changes but must not persist them. If required context is missing, minimal, or insufficient, establish only the minimum confirmed truth needed for the Work. Product facts require current developer statements, authoritative product documentation, or confirmed behavior; do not infer speculative product strategy. Repository facts may be derived from authoritative repository evidence. During approved execution, a context update must be in Work scope and task-attributed; expected touchpoints guide planning but are not a path whitelist. Update context only when the Work establishes or changes durable current truth, replacing outdated truth rather than appending Work history. Inspect relevant implementation and `AGENTS.md`. After approval, translate `nerv-dev approve` through the atomic approved-plan persistence primitive: persist the complete Work contract, every approved Task contract, and activation state in one transaction. Task fields map directly from the Plan Preview: title, objective, implementation approach, expected touchpoints, acceptance criteria, and targeted validation. Use `nerv --help` only for exact primitive arguments, never to discover whether `nerv-dev` exists.
...
`nerv-dev review` is accepted only from an active Work with every Task done. It evaluates the integrated implementation, diff, validation evidence, relevant context, optional external evidence, regressions, and risks. A Nerv Review outcome exists only after the corresponding `nerv review` runtime command succeeds and persists it; narrative analysis alone is not a completed Nerv Review and must not recommend approval. Classify every finding as `critical`, `high`, `medium`, or `low`. Critical and high findings always require REWORK. Medium findings require REWORK unless the developer explicitly accepts them as durable residual risk; low findings are residual by default. Review has one Work-level outcome: PASS or REWORK. PASS persists a Git-native synthetic-tree fingerprint and is ready for optional user or external verification, then Close on request; visibly list residual low findings and accepted medium risks, which do not block Close. A PASS may become REWORK only with verification evidence and blocking findings. Every persisted REWORK, including a Review after remediation, first presents severity-labeled findings, identifies the blockers, and then proposes minimum remediation Tasks without materializing them. Each proposed remediation Task must use the same visible execution-ready structure before recommending `nerv-dev approve`. `nerv-dev approve` adds approved remediation to the same Work Item and reactivates it; execution then stops before the next explicit Work Review.
...
- Preserve Product Context and Repo Context as canonical long-lived context. Nerv does not own general-purpose memory, code intelligence, or discovery records.
