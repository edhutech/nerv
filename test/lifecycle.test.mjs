...
import { activeContextPath, currentRef, Database, assert, existsSync, finish, join, materialize, minimalPlan, plan, readFileSync, remediation, review, rmSync, run, setup, writeFileSync } from "./helpers.mjs";
...
test("Close requires PASS review", () => {
  ...
  try {
    ...
    assert(run(repo, ["close", ref, "--message", "early"], 1).includes("not ready"), "Close did not require PASS");
  } finally {
...
test("closed Work status is terminal and has no lifecycle recommendation", () => {
  ...
  try {
    ...
    finish(repo, 2, "two.txt");
    review(repo, "PASS");
    const closed = run(repo, ["close", ref, "--message", "close"]);
    ...
    const status = run(repo, ["work", "status", ref]);
...
test("materialization keeps Task progression operational for one and multiple Tasks", () => {
  for (const value of [minimalPlan("One Task"), plan("Multiple Tasks")]) {
    ...
    try {
      ...
      assert(run(repo, ["work", "status", ref]).includes("Execution: Task 1 is active; completing it activates the next Task."), "status omitted operational Task state");
    } finally {
      rmSync(repo, { recursive: true, force: true });
    }
  }
...
test("tasks activate and progress automatically through rework", () => {
  ...
  try {
    ...
    review(repo, "PASS");
    run(repo, ["close", ref, "--message", "close"]);
    assert(!existsSync(activeContextPath(repo)), "Close did not remove active context");
  } finally {
    rmSync(repo, { recursive: true, force: true });
  }
});
...
test("blank Task, Review, and Checkpoint evidence fail without lifecycle mutation", () => {
  ...
  const snapshot = () => {
    ...
    const value = {
      work: db.prepare("SELECT status, validation_evidence FROM work_items WHERE ref=?").get(ref),
      tasks: db.prepare("SELECT position, status, validation_evidence, attribution_json FROM tasks WHERE work_item_id=(SELECT id FROM work_items WHERE ref=?) ORDER BY position").all(ref),
  ...
  try {
    ...
    assert(run(repo, ["work", "task", "done", ref, "1", "--evidence", "   "], 1).includes("must be non-empty"), "blank Task evidence was accepted");
    ...
    assert(run(repo, ["checkpoint", ref, "--summary", "   "], 1).includes("must be non-empty"), "blank Checkpoint summary was accepted");
    ...
    assert(run(repo, ["review", ref, "--outcome", "PASS", "--summary", "", "--validation-evidence", "full"], 1).includes("must be non-empty"), "blank Review summary was accepted");
    ...
    run(repo, ["checkpoint", ref, "--summary", " valid checkpoint "]);
    const checkpoint = snapshot();
    assert(checkpoint.checkpoints.count === 1, "valid Checkpoint summary was not persisted");
    assert(run(repo, ["work", "show", ref]).includes("Summary: valid checkpoint"), "valid Checkpoint summary was not trimmed and shown");
    assert(run(repo, ["review", ref, "--outcome", "PASS", "--summary", " valid review ", "--validation-evidence", "full"]).includes("PASS"), "valid Review summary was rejected");
    assert(run(repo, ["work", "show", ref]).includes("Summary: valid review"), "valid Review summary was not trimmed and shown");
  } finally {
    rmSync(repo, { recursive: true, force: true });
  }
});
