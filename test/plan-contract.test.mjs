...
test("minimal approved plans persist optional fields as empty strings", () => {
  ...
  try {
    ...
    const shown = run(repo, ["work", "show", currentRef(repo)]);
...
test("materialize help exposes a parser-valid typed plan contract", () => {
  ...
  try {
    ...
    assert(run(repo, ["work", "show", currentRef(repo)]).includes("Add status"), "documented materialize example was not accepted");
  } finally {
    rmSync(repo, { recursive: true, force: true });
  }
});
