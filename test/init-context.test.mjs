...
import { Database, assert, currentRef, finish, git, gitResult, join, materialize, materializedRef, mkdtempSync, plan, readFileSync, remediation, review, rmSync, run, setup, tmpdir, writeFileSync } from "./helpers.mjs";
...
test("canonical context blocks new Work but permits attributed rework", () => {
  ...
  for (const path of [".nerv-context/product.md", ".nerv-context/repo.md", ".agents/skills/nerv/SKILL.md"]) { const drift = setup(); try { materialize(drift); finish(drift, 1, "one.txt"); finish(drift, 2, "two.txt"); review(drift, "PASS"); run(drift, ["close", currentRef(drift), "--message", "close"]); writeFileSync(join(drift, path), "manual drift\n"); assert(run(drift, ["work", "materialize", "--plan", JSON.stringify(plan("next"))], 1).includes("setup/context must be committed"), `${path} drift did not block next Work`); git(drift, ["add", path]); git(drift, ["commit", "-m", "resolve canonical drift"]); assert(materialize(drift, plan("next")), `${path} resolution did not permit next Work`); } finally { rmSync(drift, { recursive: true, force: true }); } }
  const rework = setup(); try { materialize(rework); finish(rework, 1, "one.txt"); finish(rework, 2, "two.txt"); review(rework, "REWORK", remediation); const ref = currentRef(rework); const recovery = run(rework, ["work", "show", ref]); assert(recovery.includes("Persisted remediation proposal") && recovery.includes("Fix") && recovery.includes("Expected touchpoints: src/index.ts"), "fresh REWORK recovery omitted the persisted remediation contract"); writeFileSync(join(rework, ".nerv-context/product.md"), "canonical drift\n"); run(rework, ["work", "materialize-rework", ref]); const db = new Database(join(rework, ".nerv/nerv.db"), { readOnly: true }); const item = db.prepare("SELECT status FROM work_items ORDER BY created_at DESC LIMIT 1").get(); const tasks = db.prepare("SELECT position, status FROM tasks WHERE work_item_id=(SELECT id FROM work_items ORDER BY created_at DESC LIMIT 1) ORDER BY position").all(); const count = db.prepare("SELECT COUNT(*) AS count FROM work_items").get().count; db.close(); assert(item.status === "active" && count === 1 && JSON.stringify(tasks) === JSON.stringify([{ position: 1, status: "done" }, { position: 2, status: "done" }, { position: 3, status: "active" }]) && readFileSync(join(rework, ".nerv-context/product.md"), "utf8") === "canonical drift\n", "canonical drift blocked or normalized same-Work remediation"); writeFileSync(join(rework, "fix.txt"), "feature\n"); run(rework, ["work", "task", "done", ref, "3", "--evidence", "targeted", "--files", "fix.txt", ".nerv-context/product.md"]); review(rework, "PASS"); run(rework, ["close", ref, "--message", "close rework"]); assert(materialize(rework, plan("next")), "attributed canonical drift did not permit a subsequent Work"); } finally { rmSync(rework, { recursive: true, force: true }); }
});
