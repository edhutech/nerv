...
import { activeContextPath, assert, currentRef, existsSync, finish, join, materialize, plan, readFileSync, remediation, review, rmSync, root, run, setup, writeFileSync } from "./helpers.mjs";
...
test("active context is a compact handoff and work show retains durable detail", () => {
  ...
  try {
    ...
    const shown = run(repo, ["work", "show", ref]);
    ...
    run(repo, ["work", "task", "done", ref, "1", "--evidence", "targeted passed", "--files", "one.txt"]);
    ...
    assert(run(repo, ["work", "show", ref]).includes("Persisted remediation proposal") && run(repo, ["work", "show", ref]).includes("Fix"), "rework recovery omitted persisted remediation");
