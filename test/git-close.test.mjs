...
import { activeContextPath, assert, chmodSync, currentRef, Database, existsSync, finish, git, gitResult, join, materialize, mkdirSync, mkdtempSync, plan, readFileSync, review, rmSync, run, setup, tmpdir, writeFileSync } from "./helpers.mjs";
...
test("Close preserves special entries, exact reviewed trees, literal paths, deletions, and no-diff behavior", () => {
  const special = setup(); const indexDir = mkdtempSync(join(tmpdir(), "nerv-special-index-")); try {
    ...
    materialize(special); finish(special, 1, "one.txt"); finish(special, 2, "two.txt"); review(special, "PASS"); run(special, ["close", currentRef(special), "--message", "preserve special Git entries"]);
  ...
  const paths = setup(); try {
    ...
    materialize(paths); writeFileSync(join(paths, "space name.txt"), "feature\n"); rmSync(join(paths, "literal[1].txt")); run(paths, ["work", "task", "done", currentRef(paths), "1", "--evidence", "exact paths", "--files", "space name.txt", "literal[1].txt"]); finish(paths, 2, "two.txt"); review(paths, "PASS"); run(paths, ["close", currentRef(paths), "--message", "literal paths"]);
  ...
  const magic = setup(); const magicIndexDir = mkdtempSync(join(tmpdir(), "nerv-literal-path-index-")); try {
    ...
    if (process.platform === "win32") {
      ...
      assert(inserted.status === 0, "POSIX Git rejected representable pathspec-magic fixture"); const tree = git(magic, ["write-tree"], env).stdout.trim(); const commit = git(magic, ["commit-tree", tree, "-p", head, "-m", "literal Git path"]).stdout.trim(); git(magic, ["update-ref", "HEAD", commit, head]); git(magic, ["read-tree", "HEAD"]); git(magic, ["update-index", "--skip-worktree", "--", ":(glob)literal.txt"], { GIT_LITERAL_PATHSPECS: "1" });
        materialize(magic); run(magic, ["work", "task", "done", currentRef(magic), "1", "--evidence", "literal magic", "--files", ":(glob)literal.txt"]); finish(magic, 2, "two.txt"); review(magic, "PASS"); run(magic, ["close", currentRef(magic), "--message", "literal magic"]);
  ...
  const deletion = setup(); try { writeFileSync(join(deletion, "delete.txt"), "base\n"); git(deletion, ["add", "delete.txt"]); git(deletion, ["commit", "-m", "deletion base"]); materialize(deletion); rmSync(join(deletion, "delete.txt")); run(deletion, ["work", "task", "done", currentRef(deletion), "1", "--evidence", "deletion", "--files", "delete.txt"]); finish(deletion, 2, "two.txt"); review(deletion, "PASS"); run(deletion, ["close", currentRef(deletion), "--message", "deletion"]); assert(!existsSync(join(deletion, "delete.txt")), "exact deletion was not committed"); } finally { rmSync(deletion, { recursive: true, force: true }); }
  const noDiff = setup(); try { materialize(noDiff); run(noDiff, ["work", "task", "done", currentRef(noDiff), "1", "--evidence", "no tracked change"]); run(noDiff, ["work", "task", "done", currentRef(noDiff), "2", "--evidence", "no tracked change"]); const before = git(noDiff, ["rev-parse", "HEAD"]).stdout.trim(); review(noDiff, "PASS"); run(noDiff, ["close", currentRef(noDiff), "--message", "no diff"]); const db = new Database(join(noDiff, ".nerv/nerv.db"), { readOnly: true }); try { assert(git(noDiff, ["rev-parse", "HEAD"]).stdout.trim() === before && db.prepare("SELECT commit_hash FROM work_items ORDER BY created_at DESC LIMIT 1").get().commit_hash === null, "no-diff Close manufactured a commit"); } finally { db.close(); } } finally { rmSync(noDiff, { recursive: true, force: true }); }
});

test("Close creates the exact reviewed commit without invoking hooks", () => {
  const repo = setup(); try { materialize(repo); const ref = currentRef(repo); finish(repo, 1, "nested path.txt"); finish(repo, 2, "two.txt"); review(repo, "PASS"); const db = new Database(join(repo, ".nerv/nerv.db"), { readOnly: true }); const fingerprint = JSON.parse(db.prepare("SELECT git_fingerprint_json FROM work_reviews ORDER BY id DESC LIMIT 1").get().git_fingerprint_json); const baseline = JSON.parse(db.prepare("SELECT git_baseline_json FROM work_items ORDER BY created_at DESC LIMIT 1").get().git_baseline_json); db.close(); const hook = join(repo, ".git/hooks/pre-commit"); writeFileSync(hook, "#!/bin/sh\necho hook >> hook-mutated.txt\nexit 1\n"); chmodSync(hook, 0o755); run(repo, ["close", ref, "--message", "exact identity"]); const actual = git(repo, ["rev-parse", "HEAD"]).stdout.trim(); const closed = new Database(join(repo, ".nerv/nerv.db"), { readOnly: true }); try { const message = git(repo, ["log", "-1", "--format=%B"]).stdout; assert(git(repo, ["rev-parse", "HEAD^"]).stdout.trim() === baseline.head && git(repo, ["rev-parse", "HEAD^{tree}"]).stdout.trim() === fingerprint.tree && !existsSync(join(repo, "hook-mutated.txt")) && message.includes("Nerv-Work:") && message.includes(`Nerv-Work-Ref: ${ref}`) && closed.prepare("SELECT commit_hash FROM work_items ORDER BY created_at DESC LIMIT 1").get().commit_hash === actual, "exact-tree Close did not create or persist the reviewed commit"); } finally { closed.close(); } } finally { rmSync(repo, { recursive: true, force: true }); }
});
...
test("Close defaults to the Work title and preserves an explicit subject", () => {
  ...
  try {
    ...
    run(defaultSubject, ["close", currentRef(defaultSubject)]);
  ...
  try {
    ...
    run(suppliedSubject, ["close", currentRef(suppliedSubject), "--message", "fix: preserve subject"]);
...
test("Close rejects empty or multiline subjects before Git or lifecycle mutation", () => {
  for (const subject of ["   ", "safe\nNerv-Work: injected"]) {
    ...
    try {
      ...
      const workBefore = db.prepare("SELECT status, commit_hash FROM work_items ORDER BY created_at DESC LIMIT 1").get();
      db.close();
      const output = run(repo, ["close", currentRef(repo), "--message", subject], 1);
      ...
      const workAfter = afterDb.prepare("SELECT status, commit_hash FROM work_items ORDER BY created_at DESC LIMIT 1").get();
      afterDb.close();
      assert(git(repo, ["rev-parse", "HEAD"]).stdout.trim() === before && JSON.stringify(workAfter) === JSON.stringify(workBefore) && readFileSync(activeContextPath(repo), "utf8") === active, "invalid subject mutated Git or lifecycle state");
  ...
  try {
    ...
    const workBefore = db.prepare("SELECT status, commit_hash FROM work_items ORDER BY created_at DESC LIMIT 1").get();
    db.close();
    const active = readFileSync(activeContextPath(fallback), "utf8");
    assert(run(fallback, ["close", currentRef(fallback)], 1).includes("Close subject must be a single line"), "multiline fallback title was accepted");
    const afterDb = new Database(join(fallback, ".nerv/nerv.db"), { readOnly: true });
    const workAfter = afterDb.prepare("SELECT status, commit_hash FROM work_items ORDER BY created_at DESC LIMIT 1").get();
    afterDb.close();
    assert(git(fallback, ["rev-parse", "HEAD"]).stdout.trim() === before && JSON.stringify(workAfter) === JSON.stringify(workBefore) && readFileSync(activeContextPath(fallback), "utf8") === active, "multiline fallback title mutated Git or lifecycle state");
...
test("review findings, protected attribution, and verification downgrades remain enforced", () => {
  const remediation = ["--findings", JSON.stringify([{ severity: "high", issue: "fix", pass_impact: "The approved outcome is not met.", evidence: "Review evidence requires a fix.", affected_work_criterion: "The approved acceptance criteria." }]), "--remediation-title", "Fix", "--remediation-objective", "Resolve", "--remediation-approach", "Change", "--remediation-touchpoints", "src/index.ts", "--remediation-acceptance-criteria", "Resolved", "--remediation-validation", "pnpm test"];
  for (const severity of ["critical", "high", "medium"]) { const repo = setup(); try { materialize(repo); finish(repo, 1, "one.txt"); finish(repo, 2, "two.txt"); const findings = JSON.stringify([{ severity, issue: `${severity} issue`, pass_impact: "The approved outcome is not met.", evidence: "Review evidence requires a fix.", affected_work_criterion: "The approved acceptance criteria.", ...(severity === "medium" ? { medium_residual_risk_decision: "Not accepted because the criterion is required." } : {}) }]); assert(run(repo, ["review", currentRef(repo), "--outcome", "PASS", "--summary", "invalid", "--validation-evidence", "full", "--findings", findings], 1).includes("PASS is not permitted"), `${severity} finding did not block PASS`); assert(run(repo, ["review", currentRef(repo), "--outcome", "REWORK", "--summary", "missing proposal", "--validation-evidence", "full", "--findings", findings], 1).includes("requires title"), `${severity} REWORK accepted no proposal`); review(repo, "REWORK", ["--findings", findings, "--remediation-title", "Fix", "--remediation-objective", "Resolve issue", "--remediation-approach", "Change implementation", "--remediation-touchpoints", "src/index.ts", "--remediation-acceptance-criteria", "Issue resolved", "--remediation-validation", "pnpm test"]); run(repo, ["work", "materialize-rework", currentRef(repo)]); finish(repo, 3, "fix.txt"); assert(review(repo, "PASS").includes("PASS"), `${severity} remediation did not return to Review`); } finally { rmSync(repo, { recursive: true, force: true }); } }
  const residual = setup(); try { materialize(residual); finish(residual, 1, "one.txt"); finish(residual, 2, "two.txt"); const output = review(residual, "PASS", ["--findings", JSON.stringify([{ severity: "medium", issue: "accepted", pass_impact: "It is not accepted as a blocker.", evidence: "Review evidence.", affected_work_criterion: "A non-critical criterion.", medium_residual_risk_decision: "Accepted after review.", accepted_as_residual_risk: true }, { severity: "low", issue: "minor", pass_impact: "It does not block PASS.", evidence: "Review evidence.", affected_work_criterion: "A non-critical criterion." }])]); assert(output.includes("Residual findings") && output.includes("MEDIUM") && output.includes("LOW") && output.includes("do not block Close"), "accepted medium and low residual findings did not coexist with PASS"); } finally { rmSync(residual, { recursive: true, force: true }); }
  const protectedRepo = setup(); try { writeFileSync(join(protectedRepo, "README.md"), "preexisting change\n"); materialize(protectedRepo); assert(run(protectedRepo, ["work", "task", "done", currentRef(protectedRepo), "1", "--evidence", "unsafe", "--files", "README.md"], 1).includes("Baseline-dirty"), "protected baseline path was attributable"); } finally { rmSync(protectedRepo, { recursive: true, force: true }); }
  const nested = setup(); try { mkdirSync(join(nested, "src")); writeFileSync(join(nested, "src", "protected.txt"), "dirty\n"); materialize(nested); assert(run(nested, ["work", "task", "done", currentRef(nested), "1", "--evidence", "unsafe", "--files", "src"], 1).includes("Invalid attributable"), "directory attribution swallowed protected path"); assert(run(nested, ["work", "task", "done", currentRef(nested), "1", "--evidence", "unsafe", "--files", "src/protected.txt"], 1).includes("Baseline-dirty"), "protected nested path was attributable"); writeFileSync(join(nested, "src", "normal.txt"), "feature\n"); assert(run(nested, ["work", "task", "done", currentRef(nested), "1", "--evidence", "exact", "--files", "src/normal.txt"], 0).includes("Completed"), "exact nested sibling was rejected"); } finally { rmSync(nested, { recursive: true, force: true }); }
  const stale = setup(); try { materialize(stale); finish(stale, 1, "one.txt"); finish(stale, 2, "two.txt"); review(stale, "PASS"); writeFileSync(join(stale, "one.txt"), "changed after PASS\n"); assert(run(stale, ["close", currentRef(stale), "--message", "stale"], 1).includes("changed after PASS"), "Close accepted a changed PASS fingerprint"); assert(run(stale, ["review", currentRef(stale), "--outcome", "REWORK", "--summary", "mutated", "--validation-evidence", "full", ...remediation, "--verification-evidence", "external verification failed"], 1).includes("changed after PASS"), "mutated PASS downgraded to REWORK"); writeFileSync(join(stale, "one.txt"), "feature\n"); assert(review(stale, "REWORK", [...remediation, "--verification-evidence", "external verification failed"]).includes("REWORK"), "verification could not downgrade PASS to REWORK"); } finally { rmSync(stale, { recursive: true, force: true }); }
});
